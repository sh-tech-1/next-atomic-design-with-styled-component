...
Relative imports have been made simple. If you have to go 2 or move levels deep into a directory, use the simple import
syntax instead. The directories (and exports files) that can be accessed via simple import are as follows:
...
- Create a `.env` file in the root repository and copy over the contents from `.env.template`. Assign the right values
  to the variables.

### Command

- build the repo project on locale
